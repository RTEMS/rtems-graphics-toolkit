                       /*  ET6000 driver  Don Secrest  version 1.06     */
                       /*  version using svgalib_pci_find July 9 1999   */
...
static int  et6000_saveregs(unsigned char regs[]) /* Called frm vga_saveregs */
{
  ...
  regs[EXT + 10] = inb(base1 | 0x13);  /* I am using EXT+10 for the   */
  ...
  port_out(0x06,SEQ_I);
  regs[EXT + 5] = ((inb(base1 | 0x57) & 2) << 4); /* bit 1 of 57 to bit5  */
...
static void et6000_setregs(const unsigned char regs[], int mode)
{
  ...
  outb(base1 | 0x57,((inb(base1 | 0x57) | ((regs[EXT + 5] & 0x20) >> 4))));
...
DPRT("Found et6000\n");
		et6000_chiptype = ET6000;   /* Defined in vga.h */
		  ...
		  {
#ifdef DB6K 
		   fprintf(outf,"Can't ifnd et6000 chipset. buf[0] = 0x%8lx\n",
			(unsigned long) buf[0]);
#endif
		    return(0);
		  }
		...
		if(!pci59std) {
		  ...
		  pci40std = (unsigned char) buf[16] & 0xFF;
		  pci42std = (unsigned char) ((buf[16] & 0xFF0000) >> 16);
		  pci57std = (unsigned char) ((buf[21] & 0xFF000000) >> 24);
		  pci58std = (unsigned char) buf[22] & 0xFF;
		  pci59std = (unsigned char) ((buf[22] & 0xFF00) >> 8);
		}
		mem_cfg_reg = (unsigned char) ((buf[17] & 0xFF00) >> 8) & 3;
#ifdef DB6K
		{
		  int i;
		  for(i = 0;i < 64;i+=4)
...
static int  et6000_modeavailable(int mode)
{
  ...
  if(mode == G720x348x2 || mode == GPLANE16 || mode < G640x480x256 )
...
static void et6000_setdisplaystart(int address)
{
  ...
  outw(x3_4, 0x0d + ((address ) & 0x00ff)*256);      /* ds0-7  */
  outw(x3_4, 0x0c + ((address ) & 0xff00)*256);      /* ds0 15 */
  /* inb(x3_a);                This was from the et4000 card /? ATC write 
     outb(0x3c0,0x13 + 0x20);                                      mode.   
  outb(0x3c0,(inb(0x3c1) & 0xf0) | ((address & 3) << 1));  ds0-1  */
  outb(x3_4, 0x33);
  outb(x3_5, (inb(x3_5) & 0xfc)  | ((address & 0xf0000) >> 16 ));
                                                          /* ds 16-19 */
}
...
static void et6000_setlogicalwidth(int width)
{
  ...
  outb(x3_4 ,0x3f);
  outb(x3_5,(inb(x3_5) & 0x3f) | ((width & 0x1800) >> 3)); /* lw11-12 */
}
...
static void et6000_getmodeinfo(int mode,vga_modeinfo *modeinfo)
{
  ...
  modeinfo->startaddressrange = 0xfffff;  /* Mask for the startaddress */
...
static int  et6000_linear(int op, int param)
{
  ...
  if(op == LINEAR_QUERY_BASE) {
  ...
  else if(op == LINEAR_ENABLE)
    {
      ...
      port_out(0,0x3cb);
      port_out(5,0x3ce);
      port_out(0x40,0x3cf);  /* Set linear graphics. Don't touch this */
                             /* on disable. */
  ...
  else if(op == LINEAR_DISABLE)
    {
      ...
      /*      port_out(5,0x3ce);
      port_out(0x00,0x3cf);   See comment on LINEAR_ENABLE */
      return(0);
    }
