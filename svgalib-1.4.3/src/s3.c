...
static void s3_setpage(int page)
{
    ...
    s3_unlock();
    outb(CRT_IC, 0x35);
    outb(CRT_DC, (inb(CRT_DC) & 0xF0) | (page & 0x0F));
    if (s3_chiptype >= S3_801) {
	outb(CRT_IC, 0x51);
	outb(CRT_DC, (inb(CRT_DC) & ~0x0C) | ((page & 0x30) >> 2));
    }
    inb(CRT_DC);			/* ARI: Ferraro says: required for first generation 911 only */
    s3_lock();
}
...
static void s3_setpage864(int page)
{
    ...
    /* "Linear" mode banking. */
    outb(CRT_IC, 0x6A);
    outb(CRT_DC, (inb(CRT_DC) & ~0x3F) | page);
    s3_lock();
}
...
static void s3_setdisplaystart(int address)
{
    ...
    outb(CRT_IC, 0x31);
    outb(CRT_DC, (inb(CRT_DC) & ~0x30) | ((address & 0xc0000) >> 14));
    if (s3_chiptype >= S3_801) {
	outb(CRT_IC, 0x51);
	outb(CRT_DC, (inb(CRT_DC) & ~0x03) | ((address & 0x300000) >> 20));
    }
    s3_lock();
}
