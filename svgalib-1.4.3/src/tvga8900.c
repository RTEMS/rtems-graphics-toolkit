...
static int tvga8900_saveregs(unsigned char regs[])
{
    ...
    /* outw(SEQ_I, 0x820E); */ /* unlock conf. reg */
...
static int tvga8900_test(void)
{
    ...
    outb(SEQ_I, 0x0b);
    save0b = inb(SEQ_D);
    outw(SEQ_I, 0x000B);	/* Switch to Old Mode */
    inb(SEQ_D);			/* Now to New Mode */
    outb(SEQ_I, 0x0E);
    origVal = inb(SEQ_D);
    outb(SEQ_D, 0x00);
    newVal = inb(SEQ_D) & 0x0F;
    outb(SEQ_D, (origVal ^ 0x02));
...
static void tvga8900_setdisplaystart(int address)
{
	...
	outw(CRT_IC, 0x0d + (address & 0x00ff) * 256);
	outw(CRT_IC, 0x0c + (address & 0xff00));
    ...
    if (tvga8900_memory == 1024) {
	outw(CRT_IC, 0x0d + ((address >> 3) & 0x00ff) * 256);	/* sa2-sa9 */
	outw(CRT_IC, 0x0c + ((address >> 3) & 0xff00));	/* sa10-sa17 */
    } else {
	...
	if (tvga8900_memory == 1024) {
	    outb(ATT_IW, (inb(ATT_R) & 0xf0) | (address & 7));
  ...
  setextendedbits:
    outb(CRT_IC, 0x1e);
    outb(CRT_DC, (inb(CRT_DC) & 0x5f) | 0x80	/* set bit 7 */
	 | ((address & 0x40000) >> 13));	/* sa18: write to bit 5 */
    outb(SEQ_I, 0x0b);
    outb(SEQ_D, 0);		/* select 'old mode' */
    outb(SEQ_I, 0x0e);
    outb(SEQ_D, (inb(SEQ_D) & 0xfe)
	 | ((address & 0x80000) >> 19));	/* sa19: write to bit 0 */
    outb(SEQ_I, 0x0b);
    inb(SEQ_D);			/* return to 'new mode' */
}
