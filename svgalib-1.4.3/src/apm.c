...
void __svgalib_apmaccel_init(AccelSpecs * accelspecs, int bpp, int width_in_pixels);
...
{ unsigned long k;
    ...
    regs[APMREG_SAVE(2)] = __svgalib_inCR(0x19);
    regs[APMREG_SAVE(3)] = __svgalib_inCR(0x1A);
    regs[APMREG_SAVE(4)] = __svgalib_inCR(0x1B);
    regs[APMREG_SAVE(5)] = __svgalib_inCR(0x1C);
    regs[APMREG_SAVE(6)] = __svgalib_inCR(0x1D);
    regs[APMREG_SAVE(7)] = __svgalib_inCR(0x1E);

    regs[APMREG_SAVE(0)] = __svgalib_inSR(0x1B);
    regs[APMREG_SAVE(1)] = __svgalib_inSR(0x1C);
...
{  unsigned long k ; 
    ...
    __svgalib_outSR(0x1b,regs[APMREG_SAVE(0)]);
    __svgalib_outSR(0x1c,regs[APMREG_SAVE(1)]);
    __svgalib_outCR(0x19,regs[APMREG_SAVE(2)]);
    __svgalib_outCR(0x1a,regs[APMREG_SAVE(3)]);
    __svgalib_outCR(0x1b,regs[APMREG_SAVE(4)]);
    __svgalib_outCR(0x1c,regs[APMREG_SAVE(5)]);
    __svgalib_outCR(0x1d,regs[APMREG_SAVE(6)]);
    __svgalib_outCR(0x1e,regs[APMREG_SAVE(7)]);
...
*/
	...
	printf("svgalib(apm): Cannot get I/O permissions.\n");
	exit(-1);
    }
    ...
    /* reset iopl, apm_init will raise it again */
...
{  int i;
   ...
   */
   outb(0x3d4,0x1c);
   i=(inb(0x3d5)&0xf0)|((address & 0x0f0000) >> 16);
...
if (op==LINEAR_QUERY_BASE) {__svgalib_outSR(0x1d,0x193>>2); return inb(apm_xbase+3)<<24 ;}
...
static int apm_init(int force, int par1, int par2)
{
	...
	apm_chiptype = par2;
        ...
        /*else return 1;*/
        apm_memory=__svgalib_inSR(0x20)*64-34; /* maybe will support accel some day */
    };
    if (__svgalib_driver_report) {
	printf("Using Alliance driver, %.7s, %iKB.\n",idstring, apm_memory);
    }

    apm_xbase= (__svgalib_inSR(0x1f) << 8 ) + __svgalib_inSR(0x1e);
    ...
    __svgalib_linear_mem_base=inb(apm_xbase+3)<<24 ;
    __svgalib_linear_mem_size=apm_memory*0x400;
    return 0;
}
...
/* The XFree86 driver says the max for AT24 is 250000,
   but there is a much use clock (65MHZ), that is use 260000 fvco,
