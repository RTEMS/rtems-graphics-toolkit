...
static void gvga6400_setdisplaystart(int address)
{
    ...
    outw(CRT_IC, 0x0d + (address & 0x00ff) * 256);	/* sa0-sa7 */
    outw(CRT_IC, 0x0c + (address & 0xff00));	/* sa8-sa15 */
}
...
{
    outb(SEQ_I, 6);
    regs[EXT + 2] = inb(SEQ_D);
    outb(SEQ_I, 7);
    regs[EXT + 3] = inb(SEQ_D);
    outb(SEQ_I, 8);
    regs[EXT + 4] = inb(SEQ_D);
    outb(SEQ_I, 0x10);
    regs[EXT + 5] = inb(SEQ_D);
    outb(GRA_I, 9);
    regs[EXT + 6] = inb(GRA_D);
    outb(CRT_IC, 0x2f);
    regs[EXT + 10] = inb(CRT_DC);
...
{
    outb(SEQ_I, 6);
    outb(SEQ_D, regs[EXT + 2]);
    outb(SEQ_I, 7);
    outb(SEQ_D, regs[EXT + 3]);
    outb(SEQ_I, 8);
    outb(SEQ_D, regs[EXT + 4]);
    outb(SEQ_I, 0x10);
    outb(SEQ_D, regs[EXT + 5]);
    outb(GRA_I, 9);
    outb(GRA_D, regs[EXT + 6]);
    outb(CRT_IC, 0x2f);
    outb(CRT_DC, regs[EXT + 10]);
}
